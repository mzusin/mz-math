import { IVector2 } from './interfaces';
export declare const getV2Angle: (v2: IVector2, decimalPlaces?: number) => number;
export declare const setV2Angle: (v2: IVector2, newAngleRad: number, decimalPlaces?: number) => IVector2;
export declare const radiansToDegrees: (radians: number, decimalPlaces?: number) => number;
export declare const degreesToRadians: (degrees: number, decimalPlaces?: number) => number;
